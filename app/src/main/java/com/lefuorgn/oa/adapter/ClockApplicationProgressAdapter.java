...
public class ClockApplicationProgressAdapter extends BaseQuickAdapter<ApplyDetailsApprover> {

    public ClockApplicationProgressAdapter(List<ApplyDetailsApprover> data) {
        super(R.layout.item_activity_application_progress, data);
    }

    @Override
    protected void convert(BaseViewHolder holder, ApplyDetailsApprover approver) {
        ...
        if(position == 0) {
            ...
            holder.setVisible(R.id.iv_item_activity_application_progress, false)
                    ...
                    .setText(R.id.tv_item_activity_application_progress_status, approver.getVerify_user_name());
        }else {
            holder.setVisible(R.id.iv_item_activity_application_progress, true)
                    ...
                    .setText(R.id.tv_item_activity_application_progress_name, approver.getVerify_user_name())
                    .setText(R.id.tv_item_activity_application_progress_status, getStatus(approver.getStatus()));
            // 设置照片内容
            ...
            ImageLoader.loadImgForUserDefinedView(approver.getVerify_user_icon(), image);
        }
        long time = approver.getUpdate_time() == 0 ? approver.getCreate_time() : approver.getUpdate_time();
        holder.setText(R.id.tv_item_activity_application_progress_date,
                StringUtils.getFormatData(time, "yyyy-MM-dd HH:mm"))
                .setText(R.id.tv_item_activity_application_progress_reason, approver.getRemark())
                .setBackgroundColor(R.id.v_item_activity_application_progress, getStatusPointColor(approver.getStatus()))
                .setBackgroundColor(R.id.tv_item_activity_application_progress_status, getStatusBackgroundColor(approver.getStatus()));
    }
