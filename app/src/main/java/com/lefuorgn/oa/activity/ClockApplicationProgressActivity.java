...
import com.lefuorgn.oa.adapter.ClockApplicationProgressAdapter;
...
public class ClockApplicationProgressActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        ApplyDetails details = (ApplyDetails) getIntent().getSerializableExtra("details");
        TLog.error(details.toString());
        ApplyDetailsApplicant applicant = details.getAskForLeaveForm();
        ImageLoader.loadImgForUserDefinedView(applicant.getUser_icon(), mHeadImg);
        mNameView.setText(applicant.getUser_name());
        ClockApplicationProgressAdapter adapter = new ClockApplicationProgressAdapter(getApprovalProcesses(details));
        adapter.addFooterView(getFooter(applicant.getStatus()));
     ...
     * 获取审批流程信息列表
     */
    private List<ApplyDetailsApprover> getApprovalProcesses(ApplyDetails details) {
        List<ApplyDetailsApprover> result = details.getAskForLeaveVerifyForms();
        if(result == null) {
            result = new ArrayList<ApplyDetailsApprover>();
        }
        ApplyDetailsApplicant applicant = details.getAskForLeaveForm();
        ApplyDetailsApprover approver = new ApplyDetailsApprover();
        if(applicant != null) {
            approver.setVerify_user_name(applicant.getUser_name() + "--" + applicant.getOa_vacation_name() + "申请");
            approver.setCreate_time(applicant.getCreate_time());
        }
        result.add(0, approver);
        return result;
    }
